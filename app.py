...
def make_dip_timing_scatter(df_plot: pd.DataFrame):
    ...
    plot_df['month_label'] = trough_dt.dt.strftime('%b')
    ...
    fig.add_scatter(
        ...
        mode='markers',
        marker=dict(
            size=plot_df['size'] * 0.55,
            color=plot_df['max_drawdown_pct'].abs(),
            colorscale=[(0.0, '#f59e0b'), (1.0, '#ef4444')],
            showscale=True,
            colorbar=dict(title='|Drawdown| %', tickfont=dict(color='#f9fafb'), titlefont=dict(color='#f9fafb')),
            line=dict(color='rgba(255,255,255,0.25)', width=1),
            opacity=0.85,
        ),
        text=plot_df['year'],
        customdata=np.stack([
            plot_df['trough_date'].astype(str),
            plot_df['max_drawdown_pct'].round(2),
            plot_df['annual_return_pct'].round(2)
        ], axis=-1),
